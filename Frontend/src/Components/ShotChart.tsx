...
export const COURT_DIMENSIONS = {
  ...
  height: 677,              // 47 feet -> 677px (was 564px)
  ...
  basketY: 677,             // At baseline (was 564)
};
...
const ShotChart: React.FC = () => {
  ...
  const transformCoordinates = (x: number, y: number) => {
    ...
    return {
      x: normalizedX * width,
      y: height - (normalizedY * height) // Flip Y axis so y=490 is at top
    };
  };
  ...
  useEffect(() => {
    ...
    const loadShotData = async () => {
      ...
      try {
        ...
        console.log('Sample of all player names:', [...new Set(data.map(d => d.player_name))].slice(0, 10));
        ...
        console.log('Final sampled shots:', {
          ...
          coordinateRange: sampledShots.length > 0 ? {
  ...
  const createZonePath = (zoneName: string): string => {
    ...
    switch (zoneName) {
      case 'Restricted Area':
        ...
        return `M ${width/2 - COURT_DIMENSIONS.restrictedAreaRadius} ${height - 40} 
                A ${COURT_DIMENSIONS.restrictedAreaRadius} ${COURT_DIMENSIONS.restrictedAreaRadius} 0 0 1 ${width/2 + COURT_DIMENSIONS.restrictedAreaRadius} ${height - 40} 
                Z`;
      ...
      case 'Mid-Range': {
        ...
        return `M ${COURT_DIMENSIONS.threePointStraightDistance} ${paintBottom}
                ...
                A ${COURT_DIMENSIONS.threePointRadius} ${COURT_DIMENSIONS.threePointRadius} 0 0 1 ${arcEndX} ${arcStartY}
  ...
  const drawCourt = () => {
    ...
    const arcIntersectionY = height - 20 - Math.sqrt(threePointRadius * threePointRadius - (width/2 - threePointStraightDistance) * (width/2 - threePointStraightDistance));
    ...
    court
      .append('path')
      .attr('d', `M ${width/2 - Math.sin(arcStartAngle) * threePointRadius} ${height - 20 - Math.cos(arcStartAngle) * threePointRadius} 
                  A ${threePointRadius} ${threePointRadius} 0 0 1 ${width/2 + Math.sin(arcStartAngle) * threePointRadius} ${height - 20 - Math.cos(arcStartAngle) * threePointRadius}`)
      .attr('fill', 'none')
      .attr('stroke', '#000')
      .attr('stroke-width', 3);
    ...
    court
      .append('path')
      .attr('d', `M ${width/2 - restrictedAreaRadius} ${height - 40} 
                  A ${restrictedAreaRadius} ${restrictedAreaRadius} 0 0 1 ${width/2 + restrictedAreaRadius} ${height - 40}
                  L ${width/2 - restrictedAreaRadius} ${height - 40} Z`)
      .attr('fill', 'none')
      .attr('stroke', '#000')
      .attr('stroke-width', 2);
    ...
    const wingLineY = height - (2/5) * height;
    const leftWingEndX = width/2 - Math.sqrt(threePointRadius * threePointRadius - (height - 24 - wingLineY) * (height - 24 - wingLineY));
    const rightWingEndX = width/2 + Math.sqrt(threePointRadius * threePointRadius - (height - 24 - wingLineY) * (height - 24 - wingLineY));
    ...
    if (shotData.length > 0) {
      ...
      if (visualizationMode === 'points') {
        ...
        shotData.forEach((shot, i) => {
          ...
          if (i < 5) { // Log first 5 shots for debugging
          ...
          if (coords.x >= -50 && coords.x <= width + 50 && coords.y >= -50 && coords.y <= height + 50) {
            court
              ...
              .append('title')
              .text(`Made shot from (${shot.loc_x}, ${shot.loc_y})`);
            plotted++;
          } else {
            outOfBounds++;
          }
        ...
        console.log(`Plotted ${plotted} out of ${shotData.length} shots (${outOfBounds} out of bounds)`);
      } else {
